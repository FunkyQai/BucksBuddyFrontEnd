...
export default function NewsPage({ symbol }) {
  ...
  useEffect(() => {
    const getNews = async () => {
      try {
        const response = await axios.get("http://localhost:8000/public/asset/news/", { params: { symbol } });
          ...
          >
            ...
            <Box sx={{ display: "flex", flexDirection: "column" }}>
              ...
              <CardContent>
                ...
                <Typography variant="body2" sx={{ color: getColor(categorizeSentiment(item.sentiment_score)) }}>
                  {categorizeSentiment(item.sentiment_score)}
                </Typography>
              </CardContent>
            </Box>
            <CardContent>
